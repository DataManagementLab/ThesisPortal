...
export const load = async ({ locals, url }) => {
	...
	let archived = await db.query('SELECT * FROM topics WHERE archived = true AND author = $author ORDER BY createdAt DESC LIMIT 25 START $offset', {
		author: locals.session.cas.user,
		offset
	});
	let archivedCount = await db.query('SELECT count() AS count, archived FROM topics WHERE archived = true AND author = $author GROUP BY archived', {
		author: locals.session.cas.user
	});
	return {
